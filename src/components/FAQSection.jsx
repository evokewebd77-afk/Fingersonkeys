                        ...
                        {faqs.filter((_, i) => i % 2 === 0).map((faq) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="bg-pink-500/5 border border-pink-500/40 rounded-2xl overflow-hidden hover:border-zinc-800 hover:bg-zinc-900/40 transition-all duration-300"
                                >
                                    <button
                                        onClick={() => setActiveIndex(activeIndex === index ? null : index)}
                                        className="w-full p-6 text-left flex items-center justify-between gap-4"
                                    >
                                        <span className="text-lg font-bold text-white transition-colors group-hover:text-zinc-300">{faq.question}</span>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all flex-shrink-0 ${activeIndex === index ? 'bg-pink-500 text-white rotate-180' : 'bg-pink-500/10 text-pink-400'}`}>
                                            <ChevronDown size={20} />
                                        </div>
                                    </button>
                                    <AnimatePresence>
