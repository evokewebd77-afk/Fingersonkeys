...
const RegistrationModal = ({ isOpen, onClose, type = 'student', plan = null }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const webhookUrl = type === 'teacher'
            ...
            : 'https://damnart-ai-guladab.n8n-wsk.com/webhook/FINGER-ONKEYS(pricing)';
        ...
        try {
            ...
            const payload = {
                ...
                type,
                selectedPlan: plan ? `${plan.title} — ${plan.subtitle}` : 'General Enquiry',
                    ...
                    >
                        ...
                        <div className="mb-8 text-center relative z-10">
                            ...
                            <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight mb-3 uppercase italic leading-none font-[var(--font-brand)]">
                                {type === 'teacher' ? 'Join as ' : type === 'enquiry' ? 'Enquiry ' : 'Student '}
                                <span className="text-pink-500">{type === 'teacher' ? 'Instructor' : type === 'enquiry' ? 'Form' : 'Portal'}</span>
                            </h2>
                            <p className="text-zinc-500 text-sm">
                                {type === 'teacher'
                                    ? 'Share your musical expertise with students worldwide.'
                                    : type === 'enquiry'
                                        ? 'Fill out the form below and we will advise you on the best path.'
                        ...
                        {plan && (
                            <div className={`mb-6 px-5 py-4 rounded-2xl bg-gradient-to-r ${plan.gradient} relative z-10`}>
                                <p className="text-[10px] text-white/70 font-bold uppercase tracking-widest mb-1">Enquiring for</p>
                                <p className="text-white font-black text-lg leading-tight">{plan.title}</p>
                                <p className="text-white/80 text-xs font-semibold mt-0.5">{plan.subtitle}</p>
                            </div>
                        )}
                        ...
                        <form className="space-y-4 relative z-10" onSubmit={handleSubmit}>
                                    ...
                                    <>
                                        ...
                                        <div className="flex gap-2">
                                            <select
                                                ...
                                                onChange={handleChange}
                                                className="w-[35%] bg-white/5 border border-white/10 rounded-2xl px-2 py-4 text-white focus:outline-none focus:border-pink-500/50 focus:bg-white/[0.08] transition-all font-medium appearance-none text-center"
                                            >
                                            ...
                                            <input
                                                ...
                                                className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-pink-500/50 focus:bg-white/[0.08] transition-all placeholder:text-zinc-600 font-medium"
                                            />
                                        </div>
                                    </>
                                ) : (
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div className="w-2/3 flex gap-2">
                                                <select
                                                    ...
                                                    value={formData.countryCode}
                                                    onChange={handleChange}
                                                    className="w-[35%] w-min-20 bg-white/5 border border-white/10 rounded-2xl px-2 py-4 text-white focus:outline-none focus:border-pink-500/50 focus:bg-white/[0.08] transition-all font-medium appearance-none text-center"
                                                >
                                                    <option value="+91" className="text-black">🇮🇳 +91</option>
                                                    <option value="+1" className="text-black">🇺🇸 +1</option>
                                                    <option value="+44" className="text-black">🇬🇧 +44</option>
                                                ...
                                                <input
                                                    ...
                                                    onChange={handleChange}
                                                    placeholder="Phone"
                                                    className="w-[65%] bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-pink-500/50 focus:bg-white/[0.08] transition-all placeholder:text-zinc-600 font-medium"
                                                />
                                            </div>
                                        </div>
                                        <div className="relative group">
                                            <input
                                                ...
                                                onChange={handleChange}
                                                placeholder="Your Time Zone (e.g. IST, EST, GMT, etc.)"
                                                className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-pink-500/50 focus:bg-white/[0.08] transition-all placeholder:text-zinc-600 font-medium"
                                            />
                                        </div>
                                    </>
                            ...
                            >
                                {status === 'loading' ? (
                                    ...
                                    <>
                                        {type === 'teacher' ? 'Apply Now' : type === 'enquiry' ? 'Advise Me' : 'Join Now'}
                                        <Send size={18} />
                                    </>
                                )}
                            </button>
